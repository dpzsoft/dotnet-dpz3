...
namespace dpz3.ssr.SessionServer.Hosts {
        ...
        protected override void OnRecieveData(ServerHostRecieveEventArgs e, string command, string data) {
            ...
            switch (command) {
                ...
                case "&":
                    // 判断名称是否定义
                    if (_name == null) {
                        if (Server.IsDebug) System.Console.WriteLine($"> 名称未定义");
                    ...
                    break;
                default:
                    if (Server.IsDebug) System.Console.WriteLine($"> 未知命令类型:{command}");
        ...
        protected override void OnRecieveCommand(ServerHostRecieveEventArgs e, string command, string info) {
            ...
            switch (command) {
                ...
                case "$":
                    ...
                    if (len <= 0) {
                        if (Server.IsDebug) System.Console.WriteLine($"> 名称长度为0");
                ...
                case "&":
                    ...
                    break;
                default:
                    if (Server.IsDebug) System.Console.WriteLine($"> 未知命令类型:{command} 完整语句:{e.Content}");
                    base.SsrHost.SendFail(e, "Unknow Command");
                    break;
            }
        }

...
namespace dpz3.ssr.SessionServer {
    ...
    public class SsrHost : ssr.IHost {
        ...
        private bool CheckLogin(HostRecieveEventArgs e) {
            // 当交互存储不存在时，触发致命错误
            if (!this.IsLogin) {
                if (Server.IsDebug) System.Console.WriteLine($"> 进行{e.Content}业务时发生致命错误：尚未登录");
        ...
        private bool CheckStorageEntity(HostRecieveEventArgs e) {
            ...
            if (this.StorageEntity == null) {
                if (Server.IsDebug) System.Console.WriteLine($"> 进行{e.Content}业务时发生致命错误：交互存储不存在");
                var args = (ServerHostRecieveEventArgs)e;
        ...
        public void OnRecieve(HostRecieveEventArgs e) {
            ...
            // 日志式输出
            var args = (ServerHostRecieveEventArgs)e;
            if (Server.IsDebug) System.Console.WriteLine($"[{Time.GetTimeString()}] --> [{(args.Entity.DataMode ? "D" : "C")}] {e.Content}");
            ...
            if (_host == null) {
                switch (e.Content) {
                    ...
                    case "GET":
                        ...
                        break;
                    default:
                        if (Server.IsDebug) System.Console.WriteLine($"> 不支持的业务类型:{e.Content}");
                        break;
                }
            } else {
                // 存在业务宿主则直接将事件传递给业务宿主
        ...
        public void SendSuccess(ServerHostRecieveEventArgs e, string data = null) {
            if (data == null) {
                if (Server.IsDebug) System.Console.WriteLine($"[{Time.GetTimeString()}] <-- [+]");
                e.Entity.Send($"+0\r\n");
            } else {
                if (Server.IsDebug) System.Console.WriteLine($"[{Time.GetTimeString()}] <-- [+] {data}");
